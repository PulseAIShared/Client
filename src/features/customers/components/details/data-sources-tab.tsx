...
export const getSourceIcon = (source: string, isPrimary = false) => {
  ...
  switch (source.toLowerCase()) {
    ...
    case 'salesforce':
      return (
        <div className={`${isPrimary ? 'bg-accent-primary/20' : 'bg-accent-primary/20'} rounded-lg p-2 flex items-center justify-center`}>
    ...
    case 'pipedrive':
      return (
        <div className={`${isPrimary ? 'bg-success/20' : 'bg-success/20'} rounded-lg p-2 flex items-center justify-center`}>
    ...
    <div className="space-y-8">
          ...
          <div className="p-4 bg-warning/20 rounded-lg border border-warning/30">
            ...
            <div className="flex flex-wrap gap-2">
              {dataQuality.missingCriticalData.map((field, index) => (
                <span key={index} className="px-2 py-1 bg-warning/20 text-warning-muted rounded text-sm border border-warning/30">
                  {field}
                </span>
              ))}
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {['crm', 'payment', 'marketing', 'support', 'engagement'].map((category) => {
          ...
          return (
            <div key={category} className="bg-surface-primary backdrop-blur-lg p-6 rounded-2xl border border-border-primary shadow-lg">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  {categoryInfo.icon}
                  <div>
                    <h4 className="text-text-primary font-semibold">{categoryInfo.name}</h4>
                    <p className="text-text-muted text-sm">{categoryInfo.description}</p>
                  </div>
                </div>
              ...
              {hasData ? (
                <div className="space-y-3">
                  {sources.map((source, index) => (
                    <div key={index} className="p-3 bg-surface-secondary/30 rounded-lg border border-border-primary">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-3">
                          {getSourceIcon(source.source)}
                          <div>
                            <div className="text-text-primary font-medium capitalize">{source.source}</div>
                            <div className="text-text-muted text-xs">
                              ID: {source.externalId}
                              {source.isPrimarySource && (
                                <span className="ml-2 px-2 py-0.5 bg-accent-primary/20 text-accent-primary rounded text-xs border border-accent-primary/30">
                                  Primary
                                </span>
                              )}
                            </div>
                      ...
                      <div className="grid grid-cols-2 gap-3 text-xs">
                        <div>
                          ...
                          <div className="text-text-secondary">{formatDateTime(source.lastSyncedAt)}</div>
                        </div>
                        <div>
                          <span className="text-text-muted">Priority:</span>
                          <div className="text-text-secondary">{source.priorityLevel}</div>
                        </div>
                      </div>
                ...
                <div className="text-center py-6">
                  ...
                  <p className="text-text-muted text-sm mb-3">No {categoryInfo.name.toLowerCase()} connected</p>
                  <button className="px-4 py-2 bg-gradient-to-r from-accent-primary to-accent-secondary text-text-primary rounded-lg hover:shadow-lg transition-all duration-200 text-sm">
        ...
        <div className="bg-gradient-to-r from-accent-secondary/20 to-accent-primary/20 backdrop-blur-lg p-6 rounded-2xl border border-accent-secondary/30 shadow-lg">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {recommendations.map((rec, index) => (
              <div key={index} className="bg-surface-primary p-6 rounded-xl border border-border-primary">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h4 className="text-text-primary font-semibold mb-1">{rec.type}</h4>
                  ...
                  <div className="flex flex-col items-end gap-2">
                    ...
                    <span className="text-success text-xs font-medium">{rec.benefit}</span>
                  </div>
                </div>
                
                <div className="space-y-2">
                  <h5 className="text-text-muted text-sm font-medium">Recommended Actions:</h5>
                ...
                <button className="mt-4 w-full px-4 py-2 bg-gradient-to-r from-accent-secondary/20 to-accent-primary/20 text-accent-secondary rounded-lg hover:from-purple-600/30 hover:to-blue-600/30 transition-all duration-200 text-sm border border-accent-secondary/30">
                  Get Started
                </button>
              </div>
            ))}
      ...
      <div className="bg-surface-primary backdrop-blur-lg p-6 rounded-2xl border border-border-primary shadow-lg">
        ...
        {allDataSources.length > 0 && (
          <div className="mt-6">
            <h4 className="text-text-primary font-medium mb-4">All Connected Sources</h4>
            <div className="space-y-3">
              {allDataSources.map((source, index) => (
                <div key={index} className="flex items-center justify-between p-3 bg-surface-secondary/30 rounded-lg border border-border-primary">
                  <div className="flex items-center gap-3">
                    {getSourceIcon(source.source)}
                    <div>
                      <div className="text-text-primary font-medium flex items-center gap-2">
                        <span className="capitalize">{source.source}</span>
                        <span className="text-xs text-text-muted">({source.category.toUpperCase()})</span>
                        {source.isPrimarySource && (
                          <span className="px-2 py-0.5 bg-accent-primary/20 text-accent-primary rounded text-xs border border-accent-primary/30">
                            Primary
                          </span>
                        )}
                      </div>
                      <div className="text-text-muted text-sm">{source.externalId}</div>
                    </div>
                  </div>
